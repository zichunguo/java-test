...
public class SnowflakeIdWorker {
	...
	public synchronized long nextId() {
		...
		return ((timestamp - twepoch) << timestampLeftShift) // 时间戳的值
				| (dataCenterId << dataCenterIdShift) // 数据中心的值
				| (workerId << workerIdShift) // 工作机器的值
				| sequence;// 毫秒内序列值
	}
...
//    	return (id >> workerIdShift) & maxWorkerId;
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 1000; i++) {
			...
			System.out.println(new Date(idWorker.getTimeStamp(id)));
